// 1. Описываем базовую структуру БЕЗ аватарки
type BaseChannel = {
  id: number;
  username: string;
  new_messages: number;
...
type RawChannel = BaseChannel & { avatar?: string };
...
export type Channel = BaseChannel & { avatar: string };

const updChannels: Channel[] = channels.map((el) => ({
  ...el,
