...
## Спринт 4: WebSocket, тестирование и real-time обмен сообщениями

Реализован полноценный real-time мессенджер: обмен сообщениями через WebSocket, дозагрузка истории, автоскролл. Настроено автоматическое тестирование с Vitest — покрыты роутер, компоненты и HTTP-модуль.

### Новые возможности

#### Real-time обмен сообщениями

- **WebSocket-соединение** — отдельный контроллер `ChatWebSocketController` для управления жизненным циклом сокета (подключение, переподключение, отключение).
- **Отправка сообщений** — текст и изображения отправляются через WebSocket в реальном времени.
- **Получение сообщений** — входящие сообщения автоматически добавляются в ленту чата.
- **Дозагрузка истории** — при скролле вверх подгружаются старые сообщения (по 20 штук) через API с offset.
- **Автоскролл** — при получении нового сообщения лента автоматически прокручивается вниз, если пользователь находится в конце.
- **Сохранение позиции** — при дозагрузке старых сообщений позиция скролла сохраняется, пользователь не «прыгает» по ленте.

#### Обработка ошибок связи

- **Переподключение WebSocket** — при обрыве соединения контроллер автоматически пытается переподключиться.

#### Тестирование

- **Vitest** — настроен в качестве тестового фреймворка с окружением `jsdom` для работы с DOM.
- **Структура тестов** — файлы с тестами хранятся рядом с тестируемыми модулями.
...
| **Vitest**             | Тестирование компонентов, роутера, HTTP-модуля         |
| **WebSocket API**      | Real-time обмен сообщениями в чатах                    |
...
| `npm run test:run`  | Однократный прогон всех тестов             |
